> ⚠️ **OBSOLÈTE — archivé le 2026-07-07.** Ce document n'est plus maintenu.
> Il est conservé pour mémoire. La source de vérité est désormais le backlog
> unifié (repo NexTarget-app) : `docs/backlog/backlog-unifie.md`, projeté ici en
> `docs/specs/vue-serveur.md`. Plusieurs statuts ci-dessous sont **faux** au
> regard du code (ex. « M1/M2 supprimés / OAuth-only sans IA » alors que le proxy
> Coach IA `/coach/analyze-session` est implémenté). Voir `incoherences.md` (I1).

# Backlog v0.1 (archive)
